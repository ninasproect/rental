...
class RenlatAdvancePaymentInv(models.TransientModel):
    _name = "rental.advance.payment.inv"
    ...
    def _get_advance_payment_method(self):
        if self._count() == 1:
            rental_obj = self.env['rental.order']
            order = rental_obj.browse(self._context.get('active_ids'))[0]
            if order.rental_line.filtered(lambda dp: dp.is_downpayment) and order.invoice_ids.filtered(lambda invoice: invoice.state != 'cancel'):
                return 'all'
            else:
                return 'delivered'
        return 'all'
